...
public class RedisUtil {
  ...
  public Boolean batchDeleteKey(KeyPrefix prefix, int batchCount, String... keys) {
    ...
    String cursor =
        stringRedisTemplate.execute(
            batchRegDelete, Collections.emptyList(), ZERO, matchKey, String.valueOf(batchCount));
  ...
  public Boolean setIfAbsent(
    ...
    return redisTemplate.opsForValue().setIfAbsent(buildKey(prefix, key), value, time, timeUnit);
  }
  ...
  public Boolean batchInsert(KeyPrefix prefix, Integer count) {
    ...
    while (count - loop * batchSize >= 0) {
      ...
      IntStream.rangeClosed(loop * batchSize, count - loop * batchSize)
          .forEach(i -> map.put(buildKey(prefix, "mc_fb:user_notice_" + i), String.valueOf(i)));
        ...
        redisTemplate.executeWithStickyConnection(
            conn ->
                new ConvertingCursor<>(
                    conn.scan(scanOptions), new StringRedisSerializer()::deserialize));
    ...
    List<Long> list =
        redisTemplate.execute(
            (RedisCallback<List<Long>>)
                con -> con.bitField(buildKey(prefix, key).getBytes(), bitFieldSubCommands));
