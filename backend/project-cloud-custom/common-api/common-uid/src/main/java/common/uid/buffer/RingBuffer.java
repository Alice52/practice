...
public class RingBuffer {
     ...
     * buffer operation will be triggered in another thread<br>
     * If there is no more available UID to be taken, the specified {@link
     * RejectedTakeBufferHandler} will be applied<br>
     *
     * @return UID
    ...
    public long take() {
        ...
        Assert.isTrue(
                flags[nextCursorIndex].get() == CAN_TAKE_FLAG, "Curosr not in can take status");
        ...
        // Note that: Step 2,3 can not swap. If we set flag before get value of slot, the producer
        // may overwrite the
        // slot with a new UID, and this may cause the consumer take the UID twice after walk a
        // round the ring
        return uid;
    }
